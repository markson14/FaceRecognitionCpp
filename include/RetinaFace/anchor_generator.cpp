...
int AnchorGenerator::FilterAnchor(ulsMatF& cls, ulsMatF& reg, ulsMatF& pts, std::vector<Anchor>& result) {
    assert(cls.getChannels() == anchor_num*2);
    assert(reg.getChannels() == anchor_num*4);
    int pts_length = 0;

	assert(pts.getChannels() % anchor_num == 0);
	pts_length = pts.getChannels()/anchor_num/2;
    ...
    for (int i = 0; i < h; ++i) {
        for (int j = 0; j < w; ++j) {
            for (int a = 0; a < anchor_num; ++a)
            {
            	...
            	if (*cls.at(anchor_num + a, i, j) >= cls_threshold) {
                    ...
                    CRect2f delta(*reg.at(a*4+0, i, j),
								  *reg.at(a*4+1, i, j),
								  *reg.at(a*4+2, i, j),
								  *reg.at(a*4+3, i, j));
                    ...
                    //printf("bbox pred\n");
                    res.score = *cls.at(anchor_num + a, i, j);
                    ...
                    if (1) {
                        ...
                        for (int p = 0; p < pts_length; ++p) {
                            pts_delta[p].x = *pts.at(a*pts_length*2+p*2, i, j);
                            pts_delta[p].y = *pts.at(a*pts_length*2+p*2+1, i, j);
                        }
